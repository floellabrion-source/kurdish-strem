...
import { Play, Film, Star, Heart, ArrowRight } from 'lucide-react';
...
export default function Favorites() {
    ...
    useEffect(() => {
        ...
        axios.get('/api/movies').then(res => {
            ...
            setMovies(data.filter(m => favoriteIds.includes(m.id)));
            setLoading(false);
            ...
            <div className="home-content" style={{ paddingTop: '20px' }}>
                ...
                {loading ? (
                    ...
                    <div style={{ 
                        ...
                        <Heart size={64} color="#8b5cf6" style={{ marginBottom: '20px', opacity: 0.6 }} />
                        <h3 style={{ fontSize: '20px', color: 'var(--text)', marginBottom: '10px' }}>{t('no_favorites')}</h3>
                        <p style={{ color: 'var(--text3)', marginBottom: '20px' }}>{lang === 'en' ? 'Your favorite titles will appear here when you like them.' : 'فیلمە دڵخوازەکانت لێرەدا دەردەکەون کاتێک لایکیان دەکەیت.'}</p>
                        <button 
                            onClick={() => navigate('/')}
                            style={{
